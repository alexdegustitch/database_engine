...
public:
    BufferPool();
    Page *getPage(int id);
    void addPage(int id, const std::string &tableName, Page *p);
    void releasePage(int id);
    void evictPage(const std::string &tableName);
    void flushPage(int id);
    void flushAllPages();
    void prefetchPages(int startPage, const std::string &tableName, int cnt);
